    ...
    <div :class="{ 'modal-open ': showMOdal() }" class="modal">
        <div class="modal-box rounded-md w-11/12 max-w-max">
            <div class="flex text-lg font-semibold uppercase pb-4">
                Are you sure you want to upload this PROGRAM TABLE?
            </div>
...
export default {
    data() {
        return {
            ...
            headers: [
                ...
                {
                    title: "NAME OF SPECIFIC DISCIPLINE",
                },
                 {
                    title: "MAJOR DISCIPLINE",
                },
                 {
                    title: "MAJOR DISCIPLINE ID",
                },
            ],
            tables: [],
            showValidate: false,
    ...
    computed: {
        searchTable() {
            ...
            return this.tables
                .filter((p) => {
                    return p.B.toLowerCase().indexOf(this.search.toLowerCase()) != -1;
                })
                .slice(
    ...
    methods: {
        ...
        async storeDisciplines(disciplinesData) {
            ...
            if (this.counter === 0) {
                toast(this.counter + " Disciplines Added, data already exists.", {
                    type: TYPE.WARNING,
                    timeout: 3000,
                    hideProgressBar: true,
                    position: POSITION.TOP_RIGHT,
